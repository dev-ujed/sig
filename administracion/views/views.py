...
def certificadosIndex(request):
    if request.method == 'GET':
        if request.user.is_authenticated:
            ...
            return render(request, 'admin/certificados/index.html', {'menu': json.dumps(serializer.data)})
...
def certificadosForm(request):
    if request.method == 'GET':
        if request.user.is_authenticated:
            menu_items = MenuItem.objects.filter(parent__isnull=True)
            serializer = MenuItemSerializer(menu_items, many=True)
            return render(request, 'admin/certificados/formCertificados.html', {'menu': json.dumps(serializer.data)})
